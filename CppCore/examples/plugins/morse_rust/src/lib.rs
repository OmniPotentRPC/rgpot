...
// Descriptor returned by rgpot_plugin_init -- leaked Box for 'static lifetime

#[no_mangle]
pub extern "C" fn rgpot_plugin_init() -> *const RgpotPluginDescriptor {
    static INIT: std::sync::Once = std::sync::Once::new();
    static mut DESC: *const RgpotPluginDescriptor = std::ptr::null();

    INIT.call_once(|| {
        let d = Box::new(RgpotPluginDescriptor {
            descriptor_size: std::mem::size_of::<RgpotPluginDescriptor>(),
            api_version: RGPOT_PLUGIN_API_VERSION,
            name: b"morse-rust\0".as_ptr() as *const c_char,
