...
in.
The committed header is the canonical copy; regenerate it with =pixi r gen-
header= when the public Rust API changes.
...
The =build.rs= tries the bundled copy first, then falls back to =CppCore/rgpot/
rpc/Potentials.capnp= in the monorepo.
