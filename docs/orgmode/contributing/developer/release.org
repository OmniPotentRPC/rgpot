...
Cargo.toml=, =towncrier.toml=, =pixi.toml= (workspace key only — not
dependency pins such as nickel), and =pyproject.toml= (PyPI package
metadata).
...
and upload with =twine=. Lockstep keeps =pyproject.toml= version aligned so the
wheel metadata matches the git tag.
