...
include a /news fragment/ in the =changes/= directory:

#+begin_src bash
# Create a fragment (type: feature, bugfix, doc, removal, misc)
echo "Added DLPack tensor support to the Rust core." > changes/42.feature
#+end_src

The fragment filename is =<issue-or-pr-number>.<type>=. The pre-commit hook
checks for fragments automatically.
