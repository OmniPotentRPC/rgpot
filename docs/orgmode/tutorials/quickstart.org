#+TITLE: Quickstart
#+OPTIONS: toc:nil todo:nil num:nil author:nil title:nil
#+STARTUP: content

* Quickstart

This tutorial walks through building rgpot from source, running a Lennard-Jones
RPC server, and calling it from Python.
By the end you will have a running potential server that any Cap'n Proto client
can connect to.

** What you will need

- [[https://pixi.sh][pixi]] (manages the full toolchain; nothing else
to install)
- About 5 minutes

** Clone and build

#+begin_src bash
git clone https://github.com/OmniPotentRPC/rgpot.git
cd rgpot
pixi shell
meson setup bbdir -Dwith_tests=True --buildtype=debug
meson compile -C bbdir
#+end_src

Expected output (last few lines):

#+begin_example
[105/105] Linking target CppCore/rgpot/rpc/potserv
#+end_example

** Run the tests

#+begin_src bash
meson test -C bbdir
#+end_src

All tests should pass:

#+begin_example
Ok:                 7
Expected Fail:      0
Fail:               0
#+end_example

** Start an RPC server

Open a second terminal (or use =&=) and launch a Lennard-Jones server on port
12345:

#+begin_src bash
pixi shell   # if not already in the environment
./bbdir/CppCore/rgpot/rpc/potserv 12345 LJ
#+end_src

The server prints a line and then blocks, waiting for connections:

#+begin_example
Listening on 0.0.0.0:12345 with potential: LJ
#+end_example

** Call from Python

In the first terminal, install the Python Cap'n Proto bindings and run a
calculation:

#+begin_src bash
pixi shell -e rpctest
#+end_src

#+begin_src python
import capnp
capnp.remove_import_hook()
potentials_capnp = capnp.load("CppCore/rgpot/rpc/Potentials.capnp")

client = capnp.TwoPartyClient("localhost:12345")
pot = client.bootstrap().cast_as(potentials_capnp.Potential)

# Two hydrogen atoms separated by 2.5 Angstrom
request = pot.calculate_request()
fip = request.init("fip")
fip.pos = [0.0, 0.0, 0.0, 2.5, 0.0, 0.0]
fip.atmnrs = [1, 1]
fip.box = [20.0, 0.0, 0.0, 0.0, 20.0, 0.0, 0.0, 0.0, 20.0]

response = request.send().wait()
print(f"Energy: {response.result.energy}")
print(f"Forces: {list(response.result.forces)}")
#+end_src

Expected output:

#+begin_example
Energy: -0.02734...
Forces: [0.02148..., 0.0, 0.0, -0.02148..., 0.0, 0.0]
#+end_example

The forces are equal and opposite, as expected for a two-body LJ pair.

** What just happened

1. =potserv= loaded the built-in Lennard-Jones potential and exposed it over Cap'n
Proto RPC.
2. The Python client connected, sent atomic positions, and received energy and
forces back.
3. The same wire protocol works from any language with a Cap'n Proto implementation
(C++, Rust, Julia, Go, etc.).

** Next steps

- [[file:../reference/architecture.org][Architecture overview]] for the layer
diagram and design rationale
- [[file:../howto/integration.org][Integration guide]] for embedding rgpot into
external projects (e.g. eOn)
- [[file:../howto/embedding.org][Embedding as a subproject]] for using rgpot as a
meson/cmake dependency
- [[file:../changelog.org][Changelog]] for release history
